    ...
    public static void execute(IEventBus bus, Node node, RegistryObject<EntityType> entityObj) {
        bus.addListener((event) -> {
            if(event instanceof EntityMountEvent) {
                if(((EntityMountEvent) event).isMounting()) {
                    ...
                    if(!(((EntityMountEvent) event).getEntityBeingMounted() instanceof CustomMob) || !result.level().getEntity(result.getId()).getType().equals(entity)) return;
