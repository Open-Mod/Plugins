...
public class TemptGoal {
    public static void execute(IEventBus bus, Node node, RegistryObject<EntityType> entityObj) {
        ...
        Item item = (Item) node.getInputData("Item <Item>");
        boolean canScare = (boolean) node.getInputData("Can Scare? <Boolean>");
        mob.goalSelector.addGoal(priority, new net.minecraft.world.entity.ai.goal.TemptGoal(mob, movementSpeed, Ingredient.of(item), canScare));
